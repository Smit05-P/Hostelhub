...
function PricingCard({ plan, delay, isYearly }) {
  ...
  return (
    ...
    >
      ...
      <h3 className={`text-[24px] font-extrabold font-display ${plan.highlight ? 'text-white' : 'text-[var(--foreground)]'}`}>
        {plan.name}
      </h3>
      <p className={`mt-3 text-[15px] font-medium leading-relaxed ${plan.highlight ? 'text-slate-300' : 'text-[var(--muted-foreground)]'}`}>
        {plan.desc}
      </p>

      <div className="mt-8 flex items-baseline gap-2">
        <span className={`text-[44px] font-extrabold tracking-tight ${plan.highlight ? 'text-white' : 'text-[var(--foreground)]'}`}>
          {typeof plan.price === 'number' ? `$${plan.price}` : plan.price}
        </span>
        {typeof plan.price === 'number' && (
          <span className={`text-[16px] font-semibold ${plan.highlight ? 'text-slate-400' : 'text-[var(--muted-foreground)]'}`}>
            /month
          </span>
        )}
      </div>

      <div className={`w-full h-px my-8 ${plan.highlight ? 'bg-white/10' : 'bg-[var(--border)]'}`} />

      <ul className="flex flex-col gap-4 flex-grow mb-10">
        {plan.features.map((feat, i) => (
          <li key={i} className="flex items-center gap-3">
            <CheckCircle2 size={18} className={plan.highlight ? 'text-blue-400' : 'text-[var(--accent)]'} />
            <span className={`text-[15px] font-bold ${plan.highlight ? 'text-slate-200' : 'text-[var(--foreground)]'}`}>
              {feat}
            </span>
          </li>
        ))}
      ...
      <Button href="/register" className={`w-full h-14 rounded-2xl font-bold text-[15px] flex items-center justify-center gap-2 transition-all ${plan.highlight ? 'bg-white text-slate-900 hover:bg-slate-100 shadow-xl shadow-white/5' : 'shadow-accent'}`}>
