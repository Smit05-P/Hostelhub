...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-[100] transition-all duration-500 border-b ${isMounted ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'
        } ${scrolled
        ...
        <div className="hidden md:flex items-center gap-6 lg:gap-10">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.href}
              className={`text-[15px] text-slate-600 hover:text-[#6366f1] transition-all tracking-tight font-semibold hover:-translate-y-0.5 ${link.priority === 2 ? "hidden lg:block" : ""
                }`}
            >
              {link.name}
            </Link>
