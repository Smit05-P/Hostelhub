...
export default function StudentNoticesPage() {
  ...
  if (loading && notices.length === 0) {
    return (
      <div className="p-4 sm:p-8 max-w-6xl mx-auto min-h-screen pb-32 space-y-12 sm:space-y-16 animate-pulse">
        {/* Skeleton Header */}
        <div className="h-64 sm:h-80 bg-slate-100 rounded-[3.5rem] w-full" />
        
        {/* Skeleton Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10 mt-12">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-80 bg-slate-100 rounded-[3.5rem] w-full" />
          ))}
        </div>
      </div>
    );
