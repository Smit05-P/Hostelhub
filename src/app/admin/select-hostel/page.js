...
export default function HostelSelectionPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const { data: resp } = await axios.get(`/api/hostels?adminId=${userData?.uid || ""}`);
        const data = resp.hostels || resp || [];
        setHostels(data);
        
        // Auto-select if only one hostel and none selected yet
        if (data.length === 1 && !activeHostelId) {
          switchHostel(data[0]._id);
        }
      } catch (err) {
  ...
  const activeHostel = useMemo(() => 
    hostels.find(h => h._id === activeHostelId), 
        ...
        {filteredHostels.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-24">
            {filteredHostels.map((hostel, idx) => {
              const active = activeHostelId === hostel._id;
              return (
                <div 
                  key={hostel._id}
                ...
                >
                      ...
                      <div className="grid grid-cols-2 gap-y-6 gap-x-4">
                         ...
                         <div className="space-y-1.5">
                            ...
                            <div className="flex items-center gap-2">
                               ...
                               <span className="text-[10px] font-mono font-bold uppercase p-1 bg-white/5 rounded border border-white/10">{hostel._id.slice(-8)}</span>
                            </div>
                         </div>
                         <div className="space-y-1.5">
                   ...
                   <div className="mt-10 pt-4 flex items-center justify-between gap-4">
                      <button 
                        onClick={() => switchHostel(hostel._id)}
