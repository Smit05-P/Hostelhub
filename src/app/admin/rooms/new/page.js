    ...
    <div className="p-8 max-w-3xl mx-auto min-h-screen pb-20">
      ...
      <div className="bg-white rounded-[3rem] shadow-sm border border-slate-200/50 overflow-hidden relative">
         ...
         <form onSubmit={handleSubmit} className="p-12 space-y-10">
            ...
            {/* Duration-Based Calculation Preview */}
            {formData.price > 0 && (
               <div className="bg-blue-50/50 rounded-[2.5rem] p-8 border border-blue-100/50 space-y-6">
                  <div className="flex items-center gap-3 mb-2">
                     <div className="p-2 bg-blue-600 rounded-lg text-white">
                        <DollarSign size={16} />
                     </div>
                     <h3 className="text-xs font-black text-blue-900 uppercase tracking-widest">Total Stay Fee Calculation</h3>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                     {[
                       ...
                       { label: '4 Years', m: 48 }
                        ...
                        <div key={d.label} className="bg-white p-4 rounded-2xl border border-blue-100 shadow-sm flex flex-col items-center text-center">
                           <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mb-1">{d.label}</span>
                           <span className="text-sm font-black text-blue-700 italic">
                              ${(parseFloat(formData.price) * d.m).toLocaleString()}
                           </span>
                        </div>
                     ))}
                  </div>
                  <p className="text-[10px] font-bold text-blue-400 italic text-center">Students will be billed this total amount upon room allocation based on their selected duration.</p>
               </div>
            )}
