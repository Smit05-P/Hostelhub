...
const VisualTimeline = ({ status }) => {
  const stages = [
    { id: 'Open', label: 'Registered', icon: Send, color: 'indigo' },
...
export default function StudentComplaintsPage() {
  ...
  const { data: complaints = [], isLoading: loading, refetch: refetchComplaints } = useComplaints({ studentId: user?.uid });
  ...
  if (loading && complaints.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6">
        ...
        />
        <p className="font-black uppercase tracking-[0.4em] text-[10px] text-slate-400 italic animate-pulse">Establishing Secure Neural Link...</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-end">
        <motion.div variants={ITEM_VARIANTS} className="lg:col-span-8">
           <div className="flex items-center gap-4 mb-6">
             ...
             <div>
                <span className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.4em] italic leading-none">Security Protocol</span>
                <h1 className="text-4xl sm:text-5xl font-black text-slate-900 italic tracking-tighter mt-2 uppercase leading-none">Help Desk</h1>
             </div>
           </div>
           <p className="text-slate-500 font-bold uppercase text-[11px] tracking-[0.2em] italic max-w-xl leading-relaxed">
             ...
             Status tracking enabled for all active incident reports.
           </p>
        </motion.div>
        ...
        <motion.div variants={ITEM_VARIANTS} className="lg:col-span-4">
           <button
             ...
             className={`w-full group relative overflow-hidden flex items-center justify-center gap-4 px-10 py-6 rounded-[2rem] text-xs font-black uppercase tracking-[0.4em] transition-all transform hover:-translate-y-1 active:scale-95 shadow-2xl ${
               showForm 
           ...
           >
             ...
             {showForm ? <X size={20} /> : <Plus size={20} strokeWidth={3} />} 
             {showForm ? "ABORT OPERATION" : "INITIATE REPORT"}
           </button>
        </motion.div>
      </div>
                        ...
                        <div className="flex items-center gap-8 w-full">
                           ...
                           <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-4 mb-2">
                                 ...
                                 <div className={`p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hidden sm:block ${expandedId === complaint.id ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
                                    {expandedId === complaint.id ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                                 </div>
                              </div>
