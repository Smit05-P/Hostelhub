...
import {
  ...
  FileText, Users, IndianRupee, CheckCircle2, UserCheck,
...
const COLUMNS = [
  ...
  { key: "status",        label: "Sync Status",    sortable: true  },
];
...
export default function ReportsPage() {
  ...
  const [summary, setSummary]       = useState({ totalStudents: 0, totalCollected: 0, totalPending: 0, activeStudents: 0 });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      addToast("Matrix sync failure.", "error");
    } finally {
      setLoading(false);
    }
  }, [addToast]);
  ...
  const exportCSV = () => {
    ...
    const csvRows = sortedData.map(r => [
      r.name, r.roomNumber, r.roomType,
    ...
    >
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-10 no-print">
         ...
         <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 w-full xl:w-auto">
            ...
            <SummaryCard icon={IndianRupee} label="Collected" value={`₹${(summary.totalCollected / 1000).toFixed(1)}k`} color="bg-emerald-600" />
                    ...
                    {sortedData.length === 0 ? (
                      ...
                      sortedData.map((row, i) => (
                        ...
                        >
                           ...
                           <td className="px-10 py-8">
                              <div className="flex items-center gap-3">
                                 <div className={`w-2.5 h-2.5 rounded-full ${row.status === 'Active' ? 'bg-indigo-500 shadow-[0_0_12px_rgba(99,102,241,1)] animate-pulse' : 'bg-slate-300'}`} />
                                 <span className="text-[11px] font-black text-slate-900 uppercase italic tracking-widest">{row.status}</span>
                              </div>
                           </td>
                        </motion.tr>
                      ))
