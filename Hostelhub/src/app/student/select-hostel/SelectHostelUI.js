...
export default function SelectHostelUI() {
  const { user, role, activeHostelId, refreshStudentHostel, logout, loading: authLoading, userData } = useAuth();
  ...
  useEffect(() => {
    ...
    if (pendingCode && user?.uid) {
      sessionStorage.removeItem("pendingJoinCode");
      setJoinCode(pendingCode);
      // We wrap it in a small timeout to ensure state is updated
      setTimeout(() => {
        const fakeEvent = { preventDefault: () => {} };
        processJoin(pendingCode);
      }, 500);
    }
  }, [user]);

  const processJoin = async (codeToJoin) => {
    if (joiningCode || !codeToJoin) return;
    
    setJoiningCode(true);
    try {
      const res = await axios.post("/api/hostels/join", {
        method: "code",
        joinCode: codeToJoin.trim().toUpperCase(),
        studentId: user.uid,
      ...
      // We explicitly check for success status from backend before showing success toast
      ...
      const errorMsg = error.response?.data?.error || error.message || "Failed to join hostel. Please check your code.";
  ...
  const handleJoinByCode = async (e) => {
    ...
    if (joiningCode || !joinCode) return;
    
    // Explicitly await the process to ensure execution context remains until finished
    await processJoin(joinCode);
  };

  const handleSendRequest = async (hostelId, hostelName) => {
    if (!user?.uid || requestingHostelId) return;
    
    setRequestingHostelId(hostelId);
    try {
      const res = await axios.post("/api/hostels/join", {
        method: "request",
        hostelId,
      ...
      if (res.status === 200 || res.status === 201) {
        if (status === "approved") {
          await refreshStudentHostel(res.data.hostelId);
          addToast(message || `Successfully joined ${hostelName}!`, "success");
          setTimeout(() => {
            router.replace("/student/dashboard");
          }, 1500);
        } else if (status === "pending") {
          addToast(message || "Join request sent! Pending admin approval.", "info");
          setTimeout(() => {
            router.replace("/student/pending");
          }, 1500);
        }
      ...
      const errorMsg = error.response?.data?.error || error.message || "Failed to send join request";
      addToast(errorMsg, "error");
    } finally {
  ...
  // Content will be rendered if not redirected by ProtectedRoute

  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-jakarta overflow-x-hidden flex flex-col">
      {/* NAVBAR */}
        ...
        <div className="flex items-center gap-4 md:gap-8">
          <div className="hidden sm:flex items-center gap-4 pl-5 pr-2 py-2 bg-slate-50 border border-slate-200 rounded-full">
            <div className="flex flex-col items-end">
               ...
               <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest italic">New User</span>
            </div>
          ...
          <button 
            onClick={logout}
            aria-label="Logout"
            className="p-3 bg-slate-50 hover:bg-rose-600 text-slate-400 hover:text-white rounded-2xl border border-slate-200 hover:border-rose-600 transition-all group"
          >
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 py-12 lg:py-24 max-w-4xl mx-auto w-full">
        ...
        <div className="text-center mb-12 animate-fade-in">
          ...
          <h1 className="text-4xl md:text-6xl font-black tracking-tighter italic uppercase text-slate-900 mb-4 shadow-sm pb-2">
            Join a <span className="text-indigo-600">Hostel</span>
          </h1>
            ...
            {activeTab === "code" ? (
              <form onSubmit={handleJoinByCode} className="space-y-6 animate-fade-in w-full">
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative group">
                    <div className="absolute inset-y-0 left-6 flex items-center pointer-events-none">
                      <Navigation size={18} className="text-slate-300 group-focus-within:text-indigo-500 transition-colors" />
                    </div>
                    <input 
                      type="date" 
                      value={joiningDate}
                      onChange={(e) => setJoiningDate(e.target.value)}
                      className="w-full h-16 bg-slate-50 border border-slate-200 rounded-[1.5rem] pl-16 pr-8 text-sm font-bold text-slate-900 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all shadow-inner"
                    />
                  </div>
                </div>
                ...
                <button 
                  ...
                  className="w-full h-20 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-sm uppercase tracking-[0.3em] rounded-[2rem] flex items-center justify-center gap-3 transition-all shadow-xl shadow-indigo-600/30 active:scale-[0.98] disabled:opacity-50 disabled:active:scale-100 disabled:hover:bg-indigo-600 italic group"
                >
                  {joiningCode ? (
                    <Loader2 size={24} className="animate-spin" />
                  ) : (
                    <>Submit Key <ArrowRight size={20} className="group-hover:translate-x-2 transition-transform duration-500" /></>
                  )}
                </button>
              </form>
            ) : (
              <div className="space-y-6 animate-fade-in w-full h-full flex flex-col">
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    value={joiningDate}
                    onChange={(e) => setJoiningDate(e.target.value)}
                    className="w-full h-14 bg-slate-50 border border-slate-200 rounded-[1.5rem] px-6 text-sm font-bold text-slate-900 focus:outline-none focus:border-indigo-500 transition-all shadow-inner"
                  />
                </div>
                ...
                <div className="flex-1 bg-slate-50 border border-slate-200 rounded-[2rem] overflow-hidden p-2">
                  {searching ? (
                    ...
                    <div className="max-h-60 overflow-y-auto pr-2 space-y-2 custom-scrollbar">
                      {searchResults.map((h) => (
                        <div key={h.id} className="bg-white p-4 rounded-[1.5rem] border border-slate-100 flex items-center justify-between shadow-sm hover:shadow-md transition-shadow group">
                           <div className="flex flex-col truncate pr-4">
                             <span className="font-black italic uppercase text-slate-800 truncate tracking-tight">{h.hostelName}</span>
                             <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate">{h.address || "Location unavailable"}</span>
                           </div>
                           <button 
                             onClick={() => handleSendRequest(h.id, h.hostelName)}
                             disabled={requestingHostelId !== null}
                           ...
                           >
                              {requestingHostelId === h.id ? <Loader2 size={16} className="animate-spin" /> : "Request"}
                           </button>
                        </div>
                      ))}
                    </div>
