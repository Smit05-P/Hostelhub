...
export default function AdminLayout({ children }) {
  ...
  if (isCollectionPage) {
    return (
      <ProtectedRoute allowedRoles={["admin"]}>
        <main className={`min-h-screen bg-background grain-overlay transition-opacity duration-1000 ${isMounted ? 'opacity-100' : 'opacity-0'}`}>
          {children}
        </main>
      </ProtectedRoute>
    );
  ...
  return (
    <ProtectedRoute allowedRoles={["admin"]}>
      <div className={`flex h-screen overflow-hidden bg-background font-jakarta transition-opacity duration-1000 ${isMounted ? 'opacity-100' : 'opacity-0'}`}>
