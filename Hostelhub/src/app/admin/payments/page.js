...
export default function PaymentsPage() {
  ...
  const filteredPayments = useMemo(() => {
    return (payments || []).filter((payment) => {
      const studentName = (payment.studentName || "Unknown").toLowerCase();
                  ...
                  <tr key={payment.id} className="group hover:bg-slate-50/50 transition-all duration-300">
                    <td className="py-8 pl-10">
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          <p className="text-sm font-black text-slate-900 group-hover:text-blue-600 transition-colors uppercase tracking-tight italic">{payment.studentName || 'Unidentified'}</p>
                          <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest font-mono">Ref: {payment.id.slice(-8).toUpperCase()}</p>
                        </div>
                      </div>
                    </td>
