...
export default function RoomsPage() {
  ...
  const handleUpdateStatus = async (room, newStatus) => {
    try {
       await axios.put(`/api/rooms/${room.id}`, { ...room, status: newStatus });
                  ...
                  <div className="pt-6 sm:pt-8 border-t border-slate-50 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <button 
                        ...
                        className="p-2.5 sm:p-3 text-slate-300 hover:text-blue-600 hover:bg-white hover:shadow-2xl rounded-xl sm:rounded-2xl border border-transparent hover:border-blue-50 transition-all active:scale-90"
                      >
                        <Edit3 size={18} />
                      </button>
                      <button 
                        onClick={() => handleUpdateStatus(room, room.status === 'Maintenance' ? 'Available' : 'Maintenance')}
                        className={`p-2.5 sm:p-3 rounded-xl sm:rounded-2xl transition-all shadow-sm active:scale-90 ${room.status === 'Maintenance' ? 'text-white bg-orange-500 shadow-orange-100 border-orange-400' : 'text-slate-300 hover:text-orange-500 hover:bg-white hover:border-orange-50'}`}
                      >
                        <Settings2 size={18} />
                      </button>
                    </div>
                    <button 
                      ...
                      className="p-2.5 sm:p-3 text-slate-300 hover:text-rose-600 hover:bg-white hover:shadow-2xl rounded-xl sm:rounded-2xl border border-transparent hover:border-rose-50 transition-all active:scale-90"
                    >
                      <Trash2 size={18} />
                    </button>
                  </div>
           ...
           <div className="overflow-x-auto no-scrollbar">
             <table className="w-full text-left border-collapse">
               ...
               <tbody className="divide-y divide-slate-50">
                 {filteredRooms.map(room => (
                   <tr key={room.id} className="hover:bg-slate-50/50 transition-all duration-300 group">
                     ...
                     <td className="px-6 sm:px-10 py-8 text-right">
                        <div className="flex items-center justify-end gap-2 sm:gap-3 opacity-0 group-hover:opacity-100 transition-all duration-300">
                           ...
                           <button onClick={() => handleDelete(room.id)} className="p-2 sm:p-3 bg-white border border-slate-100 text-slate-300 hover:text-rose-600 rounded-xl sm:rounded-2xl shadow-sm hover:shadow-xl active:scale-90 transition-all"><Trash2 size={16} /></button>
                        </div>
                     </td>
                   </tr>
                 ))}
