...
export async function getAllStudents({ hostelId, search = "", filterRoom = "all", filterStatus = "all" } = {}) {
  try {
    ...
    if (filterRoom === "none") students = students.filter((s) => !s.assignedRoomId);
    if (filterStatus !== "all") students = students.filter((s) => s.status === filterStatus);
...
export async function getJoinRequests({ hostelId, status = null }) {
  try {
    let q = query(collection(db, "joinRequests"), where("hostelId", "==", hostelId));
    if (status) {
      q = query(q, where("status", "==", status));
    }
